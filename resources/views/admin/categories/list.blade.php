...
<h2>商品カテゴリ一覧</h2>
<button><a href="{{route('admin.categories.registerpage')}}">商品カテゴリ登録</a></button>
<form action="{{route('admin.categories.list')}}" method="GET">
    ...
    <table style="border: 1px black solid">
        ...
        @foreach($categories as $category)
            <tr>
                <td>{{$category->id}}</td>
                <td><a href="{{ route('admin.categories.detailpage', $category) }}">{{$category->name}}</a></td>
                <td>{{$category->created_at}}</td>
                <td><a href="{{ route('admin.categories.editpage', $category) }}">編集</a></td>
                <td><a href="{{ route('admin.categories.detailpage', $category) }}">詳細</a></td>
            </tr>
        @endforeach
    </table>
...
@if (!$return_state)
    <div class="pagination default">
        {{ $categories->links('paginate.default') }}
    </div>
